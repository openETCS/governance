...
\todo[color=yellow!20, inline]{BerndHekele: I propose to document the status of the documents also in the ReadMe of the repository. The following information is of value: Document, review status or review plan. This information can give guidance to the repository}

\todo[color=green!20, inline]{AinhoaGracia: This suggestion has also been discussed in an issue. I agree with Michael Jastram and Jan Welte in using a Wiki page to provide all the details of the ongoing reviews and maybe put in the ReadMe the updates of that wiki so everybody is aware of that. The {\it TI.6.1.5. Supervision of the Review Process} section has been updated to include this.}

\todo[color=yellow!20, inline]{BerndHekele: Abbreviation RC is missing and meaning is not clear even though the term is used all over the document}

\todo[color=green!20, inline]{AinhoaGracia: Definitions and acronyms subsection has been added.}

\begin{flushleft}

\todo[color=yellow!20, inline]{BerndHekele: we need to support also reviews of different nature, e.g., MS-Word, MS-Project, MS-Excel. In this situation I propose to make the PDF file available and make use of the "traditional" review procedure. The decision on this alternative procedure must be part of the review process, i.e., part of the invitation.}

\todo[color=yellow!20, inline]{BerndHekele: a traditional review procedure as used in the section of requirements documents should be the exception, but has to be possible for documents not reasonable in text structures.}

\todo[color=orange, inline]{AinhoaGracia: Assuming we are using GitHub we have different options: LaTex, Ms-Office or OpenOffice documents. The key point is to use the todonotes in case of LaTex documents, and an Excel sheet for other types; but not having duplicated documents. The whole process shall be controlled via GitHub and the updates with the history.}
...
\todo[color=orange, inline]{AinhoaGracia: I find this difficult to follow. In case someone is interested in assessing the evolution of a review process for a document, he/she shall look for different issues in the issue tracker without knowing the subjects. I still think the best option is using the todonotes for LaTex documents and the excel for other types of documents.}

\todo[color=yellow!20, inline]{BerndHekele: I propose we also define a section in each document (type) showing the status of the document and the documents history (like your "Document History")}

\todo[color=green!20, inline]{AinhoaGracia: Ok, I think this is a good idea. The RC manager shall be the responsible for updating the "Document History". This has been included in the TI.6.1.1 New RC branch and the TI.6.1.8 RC Pdf delivery.}
...
\todo[color=yellow!20, inline]{BerndHekele: the valid options of the todonotes tool are not documented in the proposed location}

\todo[color=green!20, inline]{AinhoaGracia: Ok. The valid options of the todonotes package have been included in the {\it Review Process} directory as a pdf}
...
\todo[color=yellow!20, inline]{BerndHekele: I see this document as a good basis for textual documents in the process. However, we also need to decide how we live the process for other documents like Code of formal specifications. We need to be prepared for those kind of documents before actual reviews start. We do not have to much time.}

\todo[color=green!20, inline]{AinhoaGracia: Of course, but I think it is good to have this Review Process for documents closed before we continue with code review and formal specification reviews}
...
\begin{minipage}{9cm}\todo[color=yellow!20, inline]{{BerndHekele: review cycles are timeboxed. When starting the cycle the review plan and the end date for the review need to be confirmed.}} \end{minipage}
...
\begin{minipage}{9cm}\todo[color=green!20, inline]{{AinhoaGracia: Ok. Item added to the section}} \end{minipage}
\end{itemize}
